...
public class LeftLightningRod {
    public void lightningRod(Player player){
        ...
        Location loc2;
        Collection<LivingEntity> nearPlayer = loc1.getNearbyLivingEntities(3, entity -> (entity instanceof LivingEntity && !(entity instanceof Player) && !(entity instanceof ArmorStand) && entity.getHealth() != 0.0D));
        Collection<LivingEntity> nearHitLoc;
        if(rayTraceResult != null){
            loc2= rayTraceResult.getHitPosition().toLocation(player.getWorld());
            nearHitLoc = loc2.getNearbyLivingEntities(2.5,entity -> (entity instanceof LivingEntity && !(entity instanceof Player) && !(entity instanceof ArmorStand) && entity.getHealth() != 0.0D));
        }else {
